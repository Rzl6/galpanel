...
- 将 README 改为适合作品集展示的精简结构；
- 突出 USB/BLE、5 Profile、EC11、45° 光标控制、状态灯和自动构建；
- 保留关键技术难点，详细过程继续链接到开发日志；
- 新增实物照片和演示 GIF 占位图，后续可直接替换；
- 编写 `v2.1.0` 与 `v2.1.1` 的简明 Release Notes；
- 将新增展示与 Release 文件纳入项目结构检查。

### 为什么这么做

项目经历页面需要让读者快速理解成果和技术含量，而不是先阅读完整开发过程。精简首页负责展示结论，详细日志继续提供工程证据。

### 发布结果

- 首页与占位图已提交并推送，提交为 `9613502`；
- 已发布 `v2.1.0`，附件为 `galpanel-v2.1.0.uf2`；
- 已发布 `v2.1.1`，附件为 `galpanel-v2.1.1.uf2`，并标记为 Latest；
- 从 GitHub Release 重新下载两个附件进行校验；
...
## 2026-09-17 - v2.1.1 对角鼠标版实物验收与版本冻结

### 实测结论

- 用户确认四个 45° 对角移动方向正确，整体功能正常；
- 按 PCB 丝印 `V2.1` 统一固件版本线；
- 原带 FN 层的长期稳定版定为 `v2.1.0`；
- 当前烧录的对角鼠标固件正式定为 GALPANEL `v2.1.1` 稳定版；
- 本轮不再修改键位、鼠标速度、EC11、SYS 或 LED 行为。

### 版本管理

- 新增仓库根目录 `VERSION`，当前内容为 `2.1.1`；
- 为原带 FN 层版本创建正式标签 `v2.1.0`；
- 将已实物验证的当前 UF2 归档为 `artifacts/backups/v2.1.1-diagonal-pointer/galpanel-v2.1.1.uf2`；
- 归档 UF2 SHA-256：`9D014DE664D94CB24DF7F42B9BD4ABBF1A11D51E94303533DC3AC0445E8AC596`；
- 版本清单记录功能代码提交 `daefe4c`、烧录记录提交 `fac87ad` 和 Actions Run `35240573008`；
- 创建 annotated Git tag `v2.1.1`，作为当前发布、恢复和差异比较的正式基准。

### 为什么这么做

“功能能用”还不足以成为可恢复版本。将实测结果、源码状态、云端构建和固件哈希绑定后，未来即使继续调整鼠标速度或键位，也可以明确回答正在使用哪个版本，并能验证恢复文件没有损坏。

### 后续版本规则

- 固件版本跟随 PCB 丝印主次版本：当前硬件为 `V2.1`，固件版本从 `v2.1.0` 起；
- 同一 PCB 上的固件功能迭代递增最后一位：带 FN 版为 `v2.1.0`，对角鼠标版为 `v2.1.1`；
- 只修复当前版本且不改变使用方式时继续递增，例如 `v2.1.2`；
- PCB 丝印升级后，固件主次版本随新硬件版本调整；
- 每个稳定版本都要记录 tag、Actions Run、UF2 SHA-256、实物测试结论，并保留独立 UF2 备份。

---

## 2026-09-17 - 四侧键改为按物理位置进行 45° 对角移动

### 本次修改

- 为已烧录的十字方向版 `cb68fc4` 创建并推送标签 `galpanel-cardinal-pointer-2026-09-17`；
- 根据实操反馈，将 D18 从左移改为左上 45°，D5 从下移改为右上 45°；
- 将 D7 从右移改为右下 45°，D14 从上移改为左下 45°；
- 每个按键使用一个 `&mmv` 行为同时输出 X/Y 位移，不使用上下左右快速交替模拟；
- D3 Esc、D19 Win+D、EC11 滚轮和 D8 SYS 均保持不变；
- 同步更新使用说明、测试计划、学习日志、稳定版本记录和项目校验规则。

### 为什么这么做

十字方向版虽然功能正确，但按键位于面板四角，实际操作时“右上键却向下”等映射需要额外记忆。让光标方向与按键物理位置完全一致，能形成更直接的空间映射，盲按更自然。

### 对后续的好处

- 四键无需记忆文字顺序，看位置即可判断移动方向；
- 单键即可沿对角线移动，适合在界面四个区域间快速定位；
- 保留十字方向 Git 标签，若对角布局不适合仍可快速回退。
...
## 2026-09-17 - 将四侧键改为默认鼠标移动模式

### 本次完成

- 在修改前为长期稳定版 `1bfcc52` 创建并推送 Git 标签 `galpanel-stable-before-mouse-mode-2026-09-17`；
- 将 Run `33605328383` 的正式 `galpanel.uf2` 备份到 `artifacts/backups/2026-09-17-stable/`，SHA-256 为 `49625D62E66C75ECA0ACF035568437FFC659CF4C3BEFBE534D3BEABA9FB43874`；
- 正式 keymap 改为单层：D18 左移、D5 下移、D7 右移、D14 上移；
- D3 从 `A` 改为 `Esc`，D19/EC11 按压从 `Esc` 改为 `Win+D`；
- 保留 D8/SYS 的 Profile、USB/BLE 切换和 bond 清除逻辑；保留 EC11 顺时针下滚、逆时针上滚；
- 删除正式固件的 QSAVE、QLOAD、侧键 Win+D tap dance 和 FN 层；
- FN 白灯改为固定熄灭，避免显示已经不存在的层状态；
- 更新使用说明、测试计划和学习日志。

### 为什么这么做

实际长期使用中，四个侧键原有宏的利用率较低，而方向移动能让 GALPANEL 在不离开主操作区的情况下承担基础鼠标导航。直接作为默认功能比额外增加模式切换更符合当前使用习惯，也避免 D14 继续占用为 FN 开关。

### 对后续的好处

- 四个侧键单击和长按都有直观用途，并支持双键斜向移动；
- 中机械键承担常用 Esc，旋钮按压承担返回桌面，手部动作更集中；
- 单层 keymap 删除了旧宏和音量层，降低误触和状态不同步风险；
- 稳定标签和本地 UF2 备份使旧版可以随时恢复，不影响已有可靠基线。
...
- 更正截图解读：`GALPANEL` 位于 Windows“新设备”区域，不是旧的已配对条目；这证明广播和名称恢复正常，但 Windows 在建立连接/配对阶段失败。
...
- 用户截图显示 Windows 蓝牙列表中的设备名仍为 `GP WIN BLE`，而 2026-07-26 通过记录中的名称是 `GALPANEL`；
- 直接检查历史 Run #3 UF2，确认二进制中不存在 `GP WIN BLE`，包含 `GALPANEL`；
- 对照源码确认 `GP WIN BLE` 只由后来的 `galpanel_win_ble_baseline` 诊断模块写入设置区；
- 确认普通 UF2 重刷不会擦除 nRF52840 设置区，因此烧录 Run #3 后名称仍可显示为 `GP WIN BLE`。

### 当前结论

本轮烧录的 Run #3 文件没有搞错；截图中的名称是前一个 Windows BLE 静态基线写入的持久化名称残留，不代表当前运行的是静态基线固件。名称问题与 BLE 配对失败是两个独立变量。

### 尚未完成

- 若要恢复历史显示名 `GALPANEL`，需要增加一个只负责覆盖持久化名称的诊断固件，再重新刷 Run #3；
- 在名称恢复前，不应把蓝牙列表中的名称作为当前固件版本判据。

### 关联提交

待提交：`docs: explain persisted Windows BLE baseline name`
...
- 用户确认当前电脑已经连接一只蓝牙鼠标和一只蓝牙键盘，卸载蓝牙驱动并重启后仍无法首次配对 GALPANEL；
- 两个已连接设备本身不会构成“只能连接两个”的硬性结论；但为排除并发干扰，下一步先临时关闭鼠标和键盘电源，不删除它们的配对记录，再尝试 GALPANEL；
- 若关闭两个现有设备后仍失败，问题进一步指向当前 Windows/适配器对该 BLE HID 设备的兼容状态，而不是连接数量。
- 需要修正测试顺序：用户在 Windows 失败后用手机成功连接，意味着当前 Run #3 的 Profile 5 现在已被手机占用；若随后不清除 Profile 5 就再次测试 Windows，设备端会按 ZMK 的单主机 Profile 规则拒绝新配对。
